...
if __name__ == '__main__':
    ...
    ####################################################################################################################################
    # Load the image without the dart
    image_no_dart = cv2.imread("/home/capstone/Flight-Path-Capstone/no_dart_in_board.jpg")

    if image_no_dart is None:
        print("Cannot read image without dart")
        exit()

    # Convert the image without dart to grayscale
    gray_no_dart = cv2.cvtColor(image_no_dart, cv2.COLOR_BGR2GRAY)

    # Perform image subtraction
    diff_image = cv2.absdiff(gray, gray_no_dart)

    # # Apply GaussianBlur to the difference image
    # blur_diff = cv2.GaussianBlur(diff_image, (5, 5), 0)

    # # Apply threshold to the difference image
    # _, thresh_diff = cv2.threshold(blur_diff, 60, 255, cv2.THRESH_BINARY)
...
########################################################################################
    ...
    blur = cv2.GaussianBlur(diff_image, (5, 5), 0) #(gray, (5, 5), 0)
    ...
    try:
        locationofdart = getRealLocation(corners_final, "left")
        ...
        cv2.circle(image, (locationofdart[0][0][0][0], locationofdart[0][0][0][1]), 10, (255, 0, 0), 2)
        cv2.circle(image, (locationofdart[0][1][0][0], locationofdart[0][1][0][1]), 2, (255, 0, 0), 2)
        # Find contours
        ...
        print("2Something went wrong in finding the dart's location!", e)
        breaker -= 1
        exit()
